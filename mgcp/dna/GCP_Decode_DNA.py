...
def GCP_Decode_DNA_brute(y, n, k, l, N, K, c1, q, len_last, lim, P2, codebook_DNA, d_min, opt = False):
    # Initializations
    uhat = None
    delta = len(y) - n
    c2 = 1
    ...
    n_check = len(codebook_DNA[0])          # number of DNA bases in the parity tail
    seg = y[-n_check:]                      # DNA tail to match
    idx, _ = find_min_levenshtein_row(codebook_DNA, seg, d_min)
    Par = [idx]                         # decimal parity symbol(s)
    p = decimal_to_binary_blocks(Par, l)[0] # return parity bits as a string of length l
    p = list(map(int, p))                   # as list[int]
    ...
    if delta == 0:
        ...
        if Y:
            ...
            if Uhat:
                ...
                if Xhat[K + c1 : K + c1 + c2] == Par and hamming_distance(Xhat, Y) <= (c1 + c2) // 2:
    ...
    for z in D:
        ...
        yE = y[:(K + c1) * (l // 2) + adjusted_delta]
        #yE = y[:(K + c1) * (l // 2) + delta]
    
        if abs(adjusted_delta) < lim:
            P = int(np.sign(adjusted_delta + np.finfo(float).eps)) * P2[abs(adjusted_delta)]
    
            num_patterns = len(P) #P.shape[0]  # Get number of rows (patterns)
    
            for i in range(num_patterns):
                lengths = np.ones(K + c1, dtype=int) * (l//2)
                ...
                Y = divide_vector_dna(yE, lengths)
                #print(Y)
                
                if Y:
                    Y.extend(Par)
                    erasure_pattern = np.zeros(N, dtype=int)  # Initialize with zeros
                    erasure_pattern[N - c2:] = 1  # Set last (c2 + flag) elements to 1
...
def GCP_Decode_DNA_rep(y, n, k, l, N, K, c1, c2, q, len_last, lim, P2, t, opt = False):
    # Initializations
    uhat = []
    delta = len(y) - n
...
def GCP_Decode_DNA_localized(y, n, k, l, N, K, c1, c2, q, len_last, P1, w, W=None):
    
    y = dna_to_binary(y)
    w = 2*w #to get equivalence for binary case
    n = 2*n
