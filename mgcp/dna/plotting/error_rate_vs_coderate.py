...
def error_rate_vs_coderate(
    ...
    cwd = os.getcwd()
    log_path = os.path.join(cwd, f"error_rate_vs_coderate_DNA_Pe{Pe}_m{marker_period}.txt")
    ...
    with open(log_path, "w", encoding="utf-8") as log_file:
        header = (
            f"--- MGC+ DNA Error Rate Vs. Code Rate Simulation ---\n"
        ...
        for parities_count in parities_count_list:
            ...
            if marker_period != 0:
                code_rate = message_length / (
                    message_length
                    + ((int(np.ceil(message_length / l)) + parities_count) / marker_period) * 4
                    + parities_count * l
                    + len(codebook[0]) * 2
                )
            else:
    ...
    plt.title(
        f"MGC+ DNA FER vs Code Rate (k={message_length}, l={l}, marker={marker_period})"
    )
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    ...
    if save_plot:
        plot_path = os.path.join(cwd, f"mgcp_dna_FERvsCoderate_Pe{Pe}_m{marker_period}.png")
