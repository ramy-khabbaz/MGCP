...
def error_rate_vs_pe(
    ...
    """
    Function to compute and plot MGC+ error rate vs Pe.
    """
    if marker_period not in (0, 1, 2):
        raise ValueError("marker_period must be 0, 1, or 2.")

    cwd = os.getcwd()
    log_path = os.path.join(cwd, "error_rate_vs_pe_log.txt")
    ...
    if marker_period != 0:
        code_rate = message_length / (
            message_length
            + ((int(math.ceil(message_length / l)) + parities_count) / marker_period) * 3
            + parities_count * l
            + len(codebook[0]) 
        )
    else:
    ...
    with open(log_path, "w", encoding="utf-8") as log_file:
        header = (
            f"--- MGC+ Error Rate Vs. Pe Simulation ---\n"
        ...
        for pe in Pe_range:
            ...
            try:
                ...
                print(f"[Warning] Multiprocessing unavailable. Running sequentially.")
    ...
    plt.title(
        f"MGC+ FER vs Pe (k={message_length}, l={l}, p={parities_count}, m={marker_period}, R={code_rate:.3f})"
    )
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    ...
    if save_plot:
        plot_path = os.path.join(cwd, f"mgcp_binary_FERvsPe_m{marker_period}.png")
