...
def MGCP_Decode_Binary_p1(y, n, l, N, K, c1, c2, q, maxSize, P0, Pd, Pi, Ps, codebook, dmin):
    # Initializations
    uhat = None
    delta = len(y) - n
    size_sub = 0
    ...
    if delta == 0:
        ...
        size_sub += 1
        yE = y[:(K + c1) * (l + 3)]
        lengths = np.ones(K + c1, dtype=int) * (l + 3)
    ...
    d = deletion_error_location(y, K, delta, l, 3, c1, P0, Pd, Pi, Ps)
