...
def CalculateProbas(m_prime, shift, p, P0, Pd, Pi, Ps):
    ...
    if m_prime == 0:
        prob = (pinfo(shift, p, Pi, Pd) * (Pr * 2 * P0 * Pi * Pd + Pi * Pd * P0 * Ps + Ps * Pr**2) +
                pinfo(shift - 1, p, Pi, Pd) * (Pr * Ps * Pi + (1 - Pd - Pi) * Pi * P0 * Ps) +
        ...
        prob = (pinfo(shift, p, Pi, Pd) * (Pd * Pi * P0 * Pr + Pr * Ps**2) +
                pinfo(shift - 1, p, Pi, Pd) * (Pd * (Pi**2) * P0) +
                pinfo(shift + 1, p, Pi, Pd) * (Pd * P0 * Pr * Ps + Pd**2 * P0 * Pi) +
        ...
        prob = (pinfo(shift, p, Pi, Pd) * (Pr * 2 * P0 * Pi * Pd + Ps * Pr**2) +
                ...
                pinfo(shift + 1, p, Pi, Pd) * (P0 * Pr * Pd * Ps * 2 + P0**2 * Pd**2 * Pi) +
                pinfo(shift + 2, p, Pi, Pd) * (P0**2 * Pd**2 * Pr))

    elif m_prime == 4:
        prob = (pinfo(shift, p, Pi, Pd) * (Pr * P0 * Pi * Pd + Ps**2 * Pr + 2 * Ps * P0 * Pi * Pd) +
                pinfo(shift - 1, p, Pi, Pd) * ((1 - Pd - Pi) * Pi * P0 * Ps) +
                pinfo(shift - 2, p, Pi, Pd) * (Ps * Pi**2 * P0) +
                pinfo(shift + 1, p, Pi, Pd) * (P0 * Pr * Pd * Ps * 2 + P0 * Pr**2 * Pd) +
                pinfo(shift + 2, p, Pi, Pd) * (P0**2 * Pd**2 * Pr * 2 + P0**2 * Pd**2 * Ps))

    elif m_prime == 5:
        prob = (pinfo(shift, p, Pi, Pd) * (Pr * 2 * P0 * Pi * Pd + Ps * Pr**2 + Ps * P0 * Pi * Pd) +
                pinfo(shift - 1, p, Pi, Pd) * ((1 - Pd - Pi) * Pi * P0 * Pr) +
        ...
        prob = (pinfo(shift, p, Pi, Pd) * (Ps**3) +
                pinfo(shift + 1, p, Pi, Pd) * (P0 * Ps**2 * Pd * 2 + P0 * Pr * Ps * Pd + 2 * Pi * Pd**2 * P0**2) +
                pinfo(shift + 2, p, Pi, Pd) * (2 * P0**2 * Pd**2 * Pr + Ps * Pd**2 * P0**2))

    elif m_prime == 7:
        prob = (pinfo(shift, p, Pi, Pd) * (Ps**2 * Pr) +
                pinfo(shift - 2, p, Pi, Pd) * (Ps * (Pi**2) * P0) +
                pinfo(shift + 1, p, Pi, Pd) * (2 * Pd * P0 * Pr * Ps + Pd**2 * P0 * Pi + Ps**2 * P0 * Pd) +
