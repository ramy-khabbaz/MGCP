...
def MGCP_Encode_p1(u, l, c1, c2, t):
    ...
    for i in range(K + c1):
        ...
        msg_withMarker += list(block) + marker

    # Append repeated check parity as DNA
    parity = list(np.repeat(check_par, t))
...
def main():
    parser = argparse.ArgumentParser(description="MGCP Encode for marker period 1")
    ...
    parser.add_argument("-t", "--repeat", type=int, required=True, help="Repeat count for check parity")
    ...
    # Convert message string to list of integers
    u = [int(bit) for bit in args.message]

    try:
        encoded, n, N, K, q, U, X = MGCP_Encode_p1(u, args.block_length, args.c1, args.c2, args.repeat)
